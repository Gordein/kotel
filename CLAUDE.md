...
- Баланс **эго-центричный** и **попарный**: прямой расчёт «кто кому» (доля участника идёт
  плательщику; при нескольких плательщиках — пропорционально), без min-transfer. Если ты в
  расчёте — ничего не показываем. `balances.py` = `compute_pairwise` + `debts_for`.
- Лента фильтруется по человеку (`?person=`), фильтр переживает автополлинг.
